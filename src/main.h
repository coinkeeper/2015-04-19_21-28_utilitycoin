...
static const int64_t MAX_MINT_PROOF_OF_STAKE = 0.15 * COIN;	// 10%
static const int64_t COIN_YEAR_REWARD = 10 * CENT; // 10% per year (output to console will be updated)
...
public:
    ...
    {
        // UtilityCoin: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64_t& nCoinAge) const;  // UtilityCoin: get transaction coin age
...
public:
    ...
    // UtilityCoin: block signature - signed by one of the coin base txout[N]'s owner
    ...
    // UtilityCoin: two types of block: proof-of-work or proof-of-stake
    ...
    // UtilityCoin: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64_t& nCoinAge) const; // UtilityCoin: calculate total coin age spent in block
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    uint256 nChainTrust; // UtilityCoin: trust score of block chain
    int nHeight;

    int64_t nMint;
    int64_t nMoneySupply;

    unsigned int nFlags;  // UtilityCoin: block index flags
    enum  
    {
