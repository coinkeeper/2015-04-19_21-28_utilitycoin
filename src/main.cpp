...
unsigned int nStakeMaxAge = 60 * 60 * 24 * 30;           // 30 days max
...
const string strMessageMagic = "UtilityCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // UtilityCoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // UtilityCoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight < 1001)
    {
        ...
        else if(pindexBest->nHeight < 1501)
    {
        nSubsidy = 1500 * COIN;
    }
        else if(pindexBest->nHeight < 2201)
    {
        nSubsidy = 1000 * COIN;
    }
        else if(pindexBest->nHeight < 2301)
    {
        nSubsidy = 2000 * COIN;
    }
    ...
    else if(pindexBest->nHeight < 3051)
    {
        nSubsidy = 1000 * COIN;
    }
    ...
    else if(pindexBest->nHeight < 4051)
    {
        nSubsidy = 1000 * COIN;
    }
    else if(pindexBest->nHeight < 5001)
    {
        nSubsidy = 200 * COIN;
    }
...
// UtilityCoin: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes UtilityCoin fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // UtilityCoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // UtilityCoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // UtilityCoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // UtilityCoin: track money supply and mint amount info
...
// UtilityCoin: total coin age spent in transaction, in the unit of coin-days.
...
// UtilityCoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // UtilityCoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // UtilityCoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // UtilityCoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // UtilityCoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // UtilityCoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // UtilityCoin: check proof-of-stake
    ...
    // UtilityCoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // UtilityCoin: getblocks may not obtain the ancestor block rejected
    ...
    // UtilityCoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "UtilityCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "The 66-satellite Iridium constellation operates as a mesh network.";
        CTransaction txNew;
        txNew.nTime = 1411673869;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x496b5b7a01f212fbdbe85cec546ecf7430a28ddf6246beb74e21a3a96d0f8b5a"));
        ...
        // UtilityCoin: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // UtilityCoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // UtilityCoin: tell downloading node about the latest block if it's
